...
class TemasController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            $user = auth()->user();
            $allowedEmails = ['admin@hoogli.com.br', 'admin@templats-link.com'];
    ...
    public function store(Request $request)
    {
        ...
        if ($temDetailBlogs) {
            \Log::info('Página detail_blogs detectada automaticamente', [
    ...
    public function renderizarPaginaDinamica($tema, $pagina, $slug = null)
    {
        try {
            ...
            if ($pagina === 'blogs') {
                $dados['blogs'] = \DB::table('posts')
                    ->where('ativo', 1)
                    ->orderBy('created_at', 'desc')
                    ->get()
                    ->map(function($post) use ($tema) {
                        return [
                            'title' => $post->titulo,
            ...
            // Para a página detail_blogs (single do post), buscar post específico
            if ($pagina === 'detail_blogs') {
...
<body>
    ...
    private function detectarPaginaDetailBlogs($request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                if (strtolower($nomeBase) === 'detail_blogs') {
                    \Log::info('Página detail_blogs encontrada no ZIP', [
                        'arquivo' => $nomeArquivo,
    ...
    private function validarRotasDinamicas($nomeTema, $paginas)
    {
        try {
            ...
            $temDetailBlogs = in_array('detail_blogs', array_map('strtolower', $paginas));
            if ($temDetailBlogs) {
                \Log::info("Página detail_blogs detectada - configurando sistema de blog", [
                    'tema' => $nomeTema,
                    'pagina' => 'detail_blogs'
                ]);
    ...
    private function configurarSistemaBlog($nomeTema)
    {
        try {
            ...
            if (!$configExistente) {
                // Criar configuração para página detail_blogs
                \DB::table('head_configs')->insert([
                    'tema' => $nomeTema,
                    'pagina' => 'detail_blogs',
            ...
            if (!$rotaDetailBlogs) {
                ...
                \Log::info("Rota detail_blogs/{slug} criada automaticamente", [
                    'tema' => $nomeTema
                ]);
            }
